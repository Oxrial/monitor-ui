...
import { getPopoverTriggerEvents, hidePopover, updatePopover } from '../utils/popovers'
...
export default {
    ...
    return h(
      'div',
      {
        class: [
          'vc-day',
          ...this.day.classes,
          { 'is-selected': this.day.isSelected },
  ...
  computed: {
    ...
    dayContentClass() {
      return ['vc-day-content vc-focusable', { 'is-disabled': this.isDisabled }, get(last(this.content), 'class') || '']
    },
    dayContentStyle() {
  ...
  methods: {
    ...
    refresh() {
      ...
      this.$set(
        this.day,
        'attributes',
        Object.values(this.day.attributesMap || {}).sort((a, b) => a.order - b.order)
      )
    ...
    processHighlight({ key, highlight }, { isDate, isComplex, onStart, onEnd, onStartAndEnd }, { backgrounds, content }) {
      if (!highlight) return
    ...
    refreshPopovers() {
      ...
      if (arrayHasItems(this.popovers)) {
        popoverEvents = getPopoverTriggerEvents(defaults({ id: this.dayPopoverId, data: this.day }, ...this.popovers))
      }
...
.vc-day {
  ...
  &.is-selected {
    ...
    background-color: #63b3ed44 !important;
  }
  &.is-not-in-month * {
    opacity: 0;
    pointer-events: none;
