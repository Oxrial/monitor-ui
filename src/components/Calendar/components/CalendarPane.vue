...
export default {
  name: 'CalendarPane',
  mixins: [childMixin, safeScopedSlotMixin, selectorMixin],
  render(h) {
    // Header
    const header =
    ...
    return h(
      ...
      [
        ...
        h(
          ...
          [this.safeScopedSlot('selector-content')]
        )
      ]
    )
  },
  ...
  computed: {
    ...
    navPopoverEvents() {
      const { sharedState, navVisibility_, navPlacement, page, position } = this
