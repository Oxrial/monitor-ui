  ...
  render(h) {
    ...
    const panes = this.pages.map((page, i) => {
      ...
      return h(CalendarPane, {
        ...
        on: {
          ...this.$listeners,
          // justRefreshPages: this.justRefreshPages,
          dayfocusin: (e) => {
    ...
    const getArrowButton = (isPrev) => {
      ...
      return h(
        'div',
        {
          class: ['vc-arrow', `is-${isPrev ? 'left' : 'right'}`, { 'is-disabled': isDisabled }],
          attrs: {
            role: 'button'
          },
          on: {
        ...
        [
          (isPrev ? this.safeScopedSlot('header-left-button', { click }) : this.safeScopedSlot('header-right-button', { click })) ||
            h(SvgIcon, {
              props: {
